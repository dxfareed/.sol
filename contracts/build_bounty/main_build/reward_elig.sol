...
contract RewardElig {
    ...
    uint256 private immutable POOL_AMOUNT; 
    ...
    uint256 public TotalClaimPerWeek;
    address[] EligibleUserArr;

    event ClaimRewardUser(address _user, uint _amount);
    ...
    uint timeAweek = block.timestamp;
    ...
    function ClaimReward() external returns (uint256) {
        ...
        //eachclaim
        if( block.timestamp < (timeAweek + 1 minutes)){
            TotalClaimPerWeek++;
        }
        else{
            TotalClaimPerWeek = 0;
            timeAweek = block.timestamp;
        }
